...
| Participant | Role |
|-------------|------|
| **AbstractClass** | Defines the template method (FINAL) and abstract/hook methods |
| **Template method (FINAL)** | Calls abstract steps in the right order |
...
- Eliminates code duplication in similar algorithms
- Enforces invariant algorithm structure
- Open/Closed

## Cons

- Template method locks subclasses to the skeleton
- Can violate Liskov if subclasses change behavior unexpectedly
...
- `Spring JdbcTemplate`
- `HTTP request lifecycle in Servlet containers`
