...
difficulty: Easy
time_complexity: "O(L) for titleToNumber, O(log_26 n) for columnToTitle"
space_complexity: "O(L) for output string"
tags: [math]
...
Excel Sheet Column Number (and Title) Convert an Excel column title (like "A", "AB", "ZY") to its corresponding column number. A→1, B→2, ..., Z→26, AA→27, AB→28, etc. Also implement the reverse: convert a column number to its title.

## Approach

titleToNumber: treat the title as a base-26 number where A=1, ..., Z=26. Process left to right: result = result * 26 + (c - 'A' + 1). For columnToTitle (reverse): like converting to base 26 but 1-indexed. While n > 0, compute remainder as (n-1) % 26 (0-indexed), prepend char ('A' + remainder), and n = (n-1) / 26.
...
- **Time:** O(L) for titleToNumber, O(log_26 n) for columnToTitle
- **Space:** O(L) for output string
