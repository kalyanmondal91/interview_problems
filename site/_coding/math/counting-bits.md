...
Counting Bits Given an integer n, return an array ans of length n+1 such that for each i in [0,n], ans[i] is the number of 1s in the binary representation of i.

## Approach

Use DP with the observation that for any number i, dp[i] = dp[i >> 1] + (i & 1). Right-shifting by 1 removes the last bit (already computed); adding (i&1) counts the LSB. This builds each answer in O(1) from a previously computed smaller value. No need to count bits individually for each number.
