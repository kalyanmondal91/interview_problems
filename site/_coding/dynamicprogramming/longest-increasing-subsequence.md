...
Longest Increasing Subsequence Given an integer array, return the length of the longest strictly increasing subsequence. A subsequence is derived by deleting some elements without changing the relative order of the remaining elements.

## Approach

O(n log n) patience sorting approach: maintain a tails array where tails[i] is the smallest tail element of all increasing subsequences of length i+1. For each number, binary search tails to find the first element >= current number. If found, replace it (smaller tail allows more future extensions); if not found, append (extend LIS). The length of tails is the LIS length.
