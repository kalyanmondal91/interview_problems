...
Add Two Numbers Given two non-empty linked lists representing two non-negative integers stored in reverse order (each node contains a single digit), add the two numbers and return the sum as a linked list, also in reverse order.

## Approach

Simulate digit-by-digit addition using a carry variable. Iterate both lists, summing corresponding digits plus carry. Each sum % 10 is the new digit, sum / 10 is the new carry. Continue until both lists are exhausted AND carry is 0. Use a dummy head to simplify pointer management.
