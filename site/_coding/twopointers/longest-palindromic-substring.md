...
Longest Palindromic Substring Given a string s, return the longest palindromic substring in s. A palindrome reads the same backward as forward. If there are multiple answers of the same maximum length, return any one of them.

## Approach

For each character (and each pair of adjacent characters for even-length palindromes), expand outward while characters on both sides match. This is the "expand around center" technique. There are 2n-1 possible centers (n single chars, n-1 pairs). Track the start and end of the longest palindrome found.
