---
layout: problem
title: "Distributed Unique I D Generator"
category: design
category_display: "Design"
difficulty: Hard
time_complexity: "O(1) amortized per ID generation"
...
Distributed Unique ID Generator (Snowflake) Generate globally unique 64-bit IDs in a distributed system. Each ID encodes a 41-bit millisecond timestamp, 10-bit machine ID, and 12-bit per-ms sequence number. IDs are monotonically increasing and sortable by time.

## Approach

On each nextId() call, get current milliseconds. If same as last, increment the 12-bit sequence (if overflow at 4095, wait for next millisecond). If different ms, reset sequence to 0. Compose result as: (timestamp << 22) | (machineId << 12) | sequence. Handle clock rollback by waiting until time catches up. Use synchronized for thread safety.
...
- **Time:** O(1) amortized per ID generation
- **Space:** O(1)
