...
difficulty: Medium
time_complexity: "O(L) for all operations where L = length of word/prefix"
space_complexity: "O(N*L) for N words of average length L"
leetcode: 208
tags: [design]
...
Implement Trie (Prefix Tree) Implement a Trie data structure with insert(word), search(word), and startsWith(prefix). search returns true if the word is in the trie. startsWith returns true if any previously inserted word has the given prefix.

## Approach

Each TrieNode contains an array of 26 child nodes (one per lowercase letter) and a boolean isEnd marking word completion. insert traverses (creating nodes as needed) and marks the last node. search traverses and returns isEnd of the last node. startsWith traverses and returns true if all characters are found regardless of isEnd.
