...
Jump Game Given an integer array nums where nums[i] represents the maximum jump length from index i, return true if you can reach the last index starting from index 0.

## Approach

Greedy: track maxReach, the farthest index reachable so far. At each position i, update maxReach = max(maxReach, i + nums[i]). If i > maxReach, we're stuck and cannot proceed. If maxReach >= n-1 at any point, we can reach the end.
