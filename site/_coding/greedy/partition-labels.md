...
difficulty: Medium
time_complexity: "O(n)"
space_complexity: "O(1) — at most 26 distinct chars"
leetcode: 763
tags: [greedy]
...
Partition Labels Given a string s, partition the string into as many parts as possible so that each letter appears in at most one part. Return a list of integers representing the size of each partition.

## Approach

First pass: record the last occurrence index of each character. Second pass: greedily extend the current partition's end to the max last occurrence of any character seen. When current index i equals the partition end, we found a complete partition — record its size and start a new partition. This ensures each letter stays in exactly one part.
