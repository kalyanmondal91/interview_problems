...
Greedy: sort intervals by end time. Maintain lastEnd (end of the last kept interval). For each interval, if its start < lastEnd, it overlaps — remove it (increment count). If no overlap, keep it and update lastEnd to this interval's end. Sorting by end ensures we greedily keep intervals that end earliest, leaving most room for future.
