...
Capacity To Ship Packages Within D Days A conveyor belt has packages that must be shipped within d days. The ith package has weight weights[i]. Each day we load packages in order with total weight not exceeding the ship's capacity. Return the minimum weight capacity of the ship that will result in shipping all packages within d days.

## Approach

Binary search on capacity in range [max(weights), sum(weights)]. For each capacity, greedily pack packages day by day checking if all fit within d days. If feasible, try smaller capacity (hi=mid); otherwise increase (lo=mid+1). The minimum feasible capacity is returned.
