...
Binary search. If nums[mid] < nums[mid+1], the peak must be in the right half (lo=mid+1) because the sequence is still ascending. Otherwise the peak is in the left half including mid (hi=mid). When lo==hi, we've found a peak. This works due to the boundary conditions.
