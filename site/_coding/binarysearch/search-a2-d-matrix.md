...
Search a 2D Matrix Write an efficient algorithm that searches for a value target in an m x n integer matrix. The matrix has the following properties: integers in each row are sorted from left to right, and the first integer of each row is greater than the last integer of the previous row.

## Approach

Treat the m x n matrix as a virtual 1D sorted array of size m*n. Binary search on virtual index [0, m*n-1]. Convert index to row = idx/n, col = idx%n. Compare matrix[row][col] with target and adjust bounds accordingly. This approach is clean and achieves O(log(m*n)).
