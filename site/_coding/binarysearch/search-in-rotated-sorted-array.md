...
Search in Rotated Sorted Array There is an integer array nums sorted in ascending order (with distinct values) that has been rotated at an unknown pivot. Given the array nums and an integer target, return the index of target if it is in nums, or -1 if it is not.

## Approach

Modified binary search. Determine which half is sorted. If nums[lo] <= nums[mid], the left half is sorted: check if target is in [nums[lo], nums[mid]). Otherwise, the right half is sorted: check if target is in (nums[mid], nums[hi]]. Narrow the search window accordingly. This achieves O(log n).
