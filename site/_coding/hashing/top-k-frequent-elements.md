...
Top K Frequent Elements Given an integer array nums and an integer k, return the k most frequent elements. The answer may be returned in any order, and it is guaranteed that the answer is unique.

## Approach

First build a frequency map using a HashMap. Then use bucket sort: create an array of lists indexed by frequency (max frequency is n). Place each element in the bucket corresponding to its frequency. Finally, iterate buckets from high to low frequency, collecting elements until we have k elements. This achieves O(n) overall.
