...
Longest Consecutive Sequence Given an unsorted array of integers nums, return the length of the longest consecutive elements sequence. The algorithm must run in O(n) time.

## Approach

Add all numbers to a HashSet for O(1) lookup. Then iterate through the array. For each number, only start counting a sequence if it is the beginning of one (i.e., num-1 is not in the set). From that start, keep incrementing and checking if the next number exists. This ensures each number is visited at most twice overall, keeping it O(n).
