...
difficulty: Easy
time_complexity: "O(n) where n is the number of words"
...
Word Pattern Given a pattern and a string s, find if s follows the same pattern. Here "follow" means a full match, such that there is a bijection between a letter in pattern and a non-empty word in s. All words in s are separated by spaces.

## Approach

Split s by spaces to get the word array. Check lengths match. Use two HashMaps: one mapping pattern chars to words, and one mapping words to pattern chars. For each pair (pattern[i], words[i]), verify consistency in both maps. If there's any conflict in either direction, return false (ensures bijection, not just injection).
