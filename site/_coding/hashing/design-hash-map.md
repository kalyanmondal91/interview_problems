...
difficulty: Easy
time_complexity: "O(n/k) average per operation, where k is number of buckets"
...
Design HashMap Design a HashMap without using any built-in hash table libraries. Implement the MyHashMap class supporting put(key, value), get(key), and remove(key) operations. Keys and values are non-negative integers.

## Approach

Use an array of LinkedLists (separate chaining). The array size is a prime number (1009) to minimize collisions. Each bucket holds key-value pair entries. For each operation, hash the key to find the bucket, then search the linked list in that bucket for the key. This provides O(1) average case for all operations.
...
- **Time:** O(n/k) average per operation, where k is number of buckets
- **Space:** O(n + k)
