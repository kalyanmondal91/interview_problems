...
Lowest Common Ancestor of a Binary Tree Given a binary tree, find the lowest common ancestor (LCA) of two given nodes p and q. The LCA is defined as the lowest node in the tree that has both p and q as descendants (where we allow a node to be a descendant of itself).

## Approach

Use recursive DFS. If the current node is null, return null. If the current node equals p or q, return the current node (it could be the ancestor). Recurse into left and right subtrees. If both left and right return non-null results, the current node is the LCA (p and q are in different subtrees). Otherwise, return whichever side is non-null.
