...
Basic Calculator Given a string s representing a valid mathematical expression containing digits, '+', '-', '(', ')' and spaces, evaluate and return the result of the expression. The expression is always valid. No multiplication or division.

## Approach

Use a stack to handle parentheses. Maintain a running result, current number, and sign (+1 or -1). When encountering '(', push the current result and sign onto the stack, reset them. When encountering ')', compute sub-expression and combine with saved values. Digits are accumulated into the current number, applied when a sign/parenthesis is met.
