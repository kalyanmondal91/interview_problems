...
Min Stack Design a stack that supports push, pop, top, and retrieving the minimum element in O(1) time. Implement the MinStack class with these four operations. The stack will always be non-empty when pop(), top(), and getMin() are called.

## Approach

Use two stacks: the main stack for regular operations, and an auxiliary minStack that records the current minimum at each state. When pushing, push the new minimum (min of val and current top of minStack) onto minStack. When popping, pop from both stacks. getMin() simply peeks the top of minStack.
