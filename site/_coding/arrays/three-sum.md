...
3Sum Given an integer array nums, return all the unique triplets [nums[i], nums[j], nums[k]] such that i != j != k and nums[i] + nums[j] + nums[k] == 0. The solution set must not contain duplicate triplets.

## Approach

Sort the array first to enable duplicate skipping and two-pointer technique. For each element (fixed as the first of the triplet), use two pointers from both ends of the remaining subarray. Skip duplicate values for the fixed element and also skip duplicates found by the two pointers after recording a valid triplet.
