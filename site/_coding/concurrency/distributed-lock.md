...
difficulty: Hard
time_complexity: "O(1) for tryLock and unlock"
space_complexity: "O(L) where L = number of distinct lock keys"
tags: [concurrency]
...
Distributed Lock Simulation Simulate a distributed lock where multiple clients compete for the same lock key. tryLock(lockKey, ownerId, ttlMillis) atomically acquires the lock if available (or if expired). unlock(lockKey, ownerId) releases the lock only if the caller is the owner.

## Approach

Use ConcurrentHashMap<lockKey, LockEntry{ownerId, expiryTime}>. tryLock() uses compute() for atomic check-and-set: if absent or TTL expired, write new entry and return true; otherwise return false. unlock() uses compute() to remove only if ownerId matches. This avoids race conditions without explicit synchronization.
...
- **Time:** O(1) for tryLock and unlock
- **Space:** O(L) where L = number of distinct lock keys
