...
space_complexity: "O(L) where L is word length"
leetcode: 79
tags: [backtracking]
...
Word Search Given an m x n grid of characters and a string word, return true if word exists in the grid. The word must be constructed from letters of sequentially adjacent cells (horizontally or vertically adjacent), and the same cell may not be used more than once.

## Approach

Iterate over every cell as a potential starting point. From each matching cell, perform DFS. Mark the current cell as visited by replacing it with '#', then recursively check all four neighbors for the next character. After recursion, restore the cell's original character. The search succeeds when all characters in the word have been matched sequentially.
...
- **Time:** O(m*n*4^L)
- **Space:** O(L) where L is word length
